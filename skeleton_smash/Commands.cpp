...
CMD_Type _processCommandLine(const char* cmd_line, char** args, int* args_count = nullptr)
{
    CMD_Type res = CMD_Type::Normal;
    std::string cmd(cmd_line);
    unsigned int pos = cmd.find_first_of("><|");
    ...
    {
        if(_isBackgroundComamnd(cmd_line)) // Insert the args after removing the ampersand.
        {
